...
struct APIPostRequest {
    ...
    func save(_ messageToSave: PostMessage, completion: @escaping(Result<PostMessage, APIPostError>) -> Void){
        do{
            ...
            let dataTask = URLSession.shared.dataTask(with: urlPostRequest) {data, response, _ in
                ...
                do{
                    ...
                    let messageData = try JSONDecoder().decode(PostMessage.self, from: jsonData)
